...
`ProjectFilesProvider` 是本地项目文件访问边界。`LocalProjectFilesProvider`
每次调用重新扫描用户明确授权的根目录，并向 Tool 返回内部、不可变的
`ProjectFile` 文本前缀；这些内部对象不进入公共 Schema 或 `AgentState`。Provider
负责真实路径解析、根目录边界检查、symlink 和 junction 处理、忽略目录、敏感文件
过滤、UTF-8 解码以及文件数、目录项、单文件和总字节限制。目录链接不递归；文件
链接只有在解析为授权根目录内的普通文件时才允许读取。

`inspect_project_evidence` 不直接访问文件系统，只消费 `ProjectFilesProvider`
返回的扫描结果。Tool 使用确定性文本规范化和匹配规则生成 `EvidenceItem`，并负责
稳定排序、数量限制、摘录脱敏、`evidence_id` 生成以及 Provider 问题到结构化
warning/error 的映射。扫描不执行、导入、编译或修改用户项目。

### Schemas

Schema 负责统一定义 `State`、请求响应对象、工具输入输出、评分报告、证据记录等结构，避免跨层自由拼装字典。

`app/schemas/tool_inputs.py` 定义六个 Tool 的强类型输入和输入级校验；
`app/schemas/tool_outputs.py` 定义成功载荷、统一失败载荷和完整返回类型。Tool
输入输出契约不持有或修改 `AgentState`。
...
- Tool 返回结构化结果，再由 Agent 写回 `State`。
...
保存用户直接提供的简历文本。它存在的原因是兼容“直接粘贴简历文本”输入路径，并与 `resume_path` 一起构成候选人资料入口。
...
保存最终投递材料和输出摘要，类型为 `ApplicationPack`。它存在的原因是区分“过程状态”和“对用户可展示的最终产物”。
...
保存当前执行状态，例如 `pending`、`running` 与各类终止状态。它存在的原因是让 API 和前端可以明确展示任务阶段与结束原因。
